...
[![Tests](https://img.shields.io/badge/Tests-97%20Passing%20(63%20Pytest%20%2B%2034%20Vitest)-success.svg)](https://github.com/kavsrd13/claimtrace)
...
|---|---|---|
| **Code Quality** | • Modular package architecture (`routers/`, `services/`, `models/`, `components/`, `pages/`)<br/>• Strict Python 3.11 type hints and Pydantic v2 schemas<br/>• SQLAlchemy 2.0 async ORM with `aiosqlite`<br/>• 100% clean Ruff linting and formatting with zero warnings | `ruff check backend/`<br/>`tsc --noEmit` |
| **Security** | • **Magic byte inspection:** Validates `%PDF-`, `PK\x03\x04` and blocks disguised executables (`MZ`, `ELF`, null bytes)<br/>• **Prompt injection defense:** Expanded regex detection for jailbreaks, system prompt overrides, and role escapes<br/>• **Rate Limiting:** Sliding-window limiter (120 req/min per IP)<br/>• **OWASP Headers:** `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`<br/>• **Path Traversal Defense:** Strict filename sanitization (`sanitize_filename`) | `test_magic_bytes.py`<br/>`test_security.py`<br/>`test_middleware.py` |
| **Efficiency** | • **SHA256 Embedding Cache:** In-memory caching eliminates redundant Azure OpenAI embedding calls<br/>• **Vectorized Cosine Similarity:** Pure NumPy matrix operations execute top-k retrieval in $< 2\text{ms}$<br/>• **Reasoning Token Budgeting:** Configured `reasoning_effort="low"` saves thousands of tokens per comparison call<br/>• **Lean Bundle:** Production JS bundle is only 60.8 KB gzipped | Production build logs<br/>`test_cache.py` |
| **Testing** | • **63 Pytest tests:** End-to-end coverage across sessions, uploads, security, magic bytes, middleware, and Q&A<br/>• **34 Vitest tests:** Component tests for ClaimTrace citation badges, keyboard interaction, empty states, and errors<br/>• **Total: 97 automated tests passing with 100% success** | `pytest backend/tests`<br/>`npm test -- --run` |
| **Accessibility (a11y)** | • **WCAG 2.1 AA Compliant:** Contrast ratios $\ge 4.5:1$<br/>• **Keyboard Navigation:** Full Tab/Shift+Tab navigation, Enter to open citations, Escape to dismiss<br/>• **Screen Readers:** `<main id="main-content">`, `<a href="#main-content" class="skip-link">`, ARIA live region (`aria-live="polite"`), explicit form labels, and table header scopes<br/>• **Reduced Motion:** `@media (prefers-reduced-motion: reduce)` support | Vitest suite<br/>Manual keyboard audit |
